...
export class SpreadsheetsService {
  async signup(data: CreateSpreadsheetData) {
    ...
    const [spreadsheet] = await db.insert(SpreadsheetsTable).values({
      uid,
      name: data.name,
      email: data.email,
  ...
  async login(data: LoginData) {
    ...
    const isValidPassword = await bcrypt.compare(data.password, spreadsheet.password);
    if (!isValidPassword) {
      throw new Error('Invalid credentials');
    }
  ...
  async getAbstractWithAnalytics(spreadsheetUid: string, mount: number, year: number) {
    ...
    return {
      ...
      analytics: {
        ...
        planned: {
          ...
          expenses: budgets.expenses.reduce((sum: number, e: any) => sum + e.planned, 0)
        },
        variance: {
          revenue: totalRevenue - budgets.revenue.planned,
          expenses: totalExpenses - budgets.expenses.reduce((sum: number, e: any) => sum + e.planned, 0)
        },
        trends: {
  ...
  private calculateProjection(actualAmount: number, currentMonth: number): number {
    if (currentMonth === 0) return 0;
    return (actualAmount / currentMonth) * 12;
  }

  async updateAbstractNotes(spreadsheetUid: string, abstractUid: string, notes: any) {
  ...
  async getRecentTransactions(spreadsheetUid: string, page: number = 1, limit: number = 25) {
    ...
    const revenuesWithType = revenues.map(r => ({ ...r, type: 'revenue' as const }));
    const expensesWithType = expenses.map(e => ({ ...e, type: 'expense' as const }));
