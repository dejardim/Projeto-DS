...
export const validateCreateAbstract = [
  body('name').notEmpty().withMessage('Name is required'),
  body('mount').isInt({ min: 1, max: 12 }).withMessage('Month must be between 1 and 12'),
  body('year').isInt({ min: 2000 }).withMessage('Year must be valid'),
  body('budgets.revenue.planned').isNumeric().withMessage('Revenue planned must be a number'),
  body('budgets.expenses').isArray().withMessage('Expenses must be an array'),
];

export const validateAbstractParams = [
  param('mount').isInt({ min: 1, max: 12 }).withMessage('Month must be between 1 and 12'),
...
export class SpreadsheetsController {
  ...
  async createAbstract(req: Request, res: Response) {
    try {
      ...
      const result = await spreadsheetsService.createAbstract(spreadsheetUid, req.body);
      res.status(201).json(result);
    } catch (error) {
  ...
  async getAbstractWithAnalytics(req: Request, res: Response) {
    try {
      ...
      if (error instanceof Error && error.message === 'Abstract not found') {
